  ...
  describe('arr', () => {
    describe('Array with single variables', () => {
      ...
      it('Should work with different datatypes', () => {
        ...
        result = f([{name: 'Alexander Van Der Bellen', country: 'Austria'}, 
          {name: 'Angela Merkel', country: 'Germany'}],
          {headers: ['president']});
        expect(isObject(result[0].president)).toBe(true);
  
        // Array:
        result = f([[1,2,3,4,5], [2,3,4,5,6]],{headers: 'count'});
    ...
    describe('Object with items', () => {
      ...
      describe('Should have a valid id', () => {
        ...
        it('Should have the give start as first id', () => {
          const result = f([{name: 'Jeff', age: 19}, {name: 'Maria', age: 20}], {start: 100});
          expect(result[0].id).toBe(100);
          expect(result[1].id).toBe(101);
        });
      });
    ...
    describe('Nested array', () => {
      ...
      it('Should return objects with headers', () => {
        const result = f([['Jeff', 19], ['Maria', 20]], {headers: ['name', 'age']});
        result.map((item) => {
          expect(item.id).toBeDefined();
          expect(typeof item.id).toBe('number');
      ...
      it('Should work with different datatypes', () => {
        // String:
        let result = f([['Jeff', 19], ['Maria', 20]], {headers: ['name', 'age']});
        expect(typeof result[0].name).toBe('string');
  
        // Number:
        result = f([['Jeff', 19], ['Maria', 20]], {headers: ['name', 'age']});
        expect(typeof result[0].age).toBe('number');
  
        // Boolean:
        result = f([[true, false, false]], {headers: ['song_01', 'song_02', 'song_03']});
        expect(typeof result[0].song_01).toBe('boolean');
  
        // Object:
        result = f([[{name: 'Jeff', age: 19}, false], [{name: 'Maria', age: 20}, true]], 
        ...
        result = f([['Mercedes CLS 350d.', 'Tesla Model X']], {headers: ['jeff_cars']});
      ...
      describe('Should have a valid id', () => {
        ...
        it('Should have a always positive id', () => {
          const result = f([['Jeff', 19], ['Maria', 20]], {headers: ['name', 'age'], start: -2});
          expect(result[0].id).toBe(1);
          expect(result[1].id).toBe(2);
        });
        it('Should have the give start as first id', () => {
          const result = f([['Jeff', 19], ['Maria', 20]], {headers: ['name', 'age'], start: 100});
          expect(result[0].id).toBe(100);
          expect(result[1].id).toBe(101);
        });
      });
  ...
  describe('parameter', () => {
    ...
    describe('increment_name', () => {
      ...
      it('Should be be a string with length > 0', () => {
        let result = f({name: 'Jeff', age: 19},
          { headers: 'person', increment_name: 'increment variable name'});
        expect(result[0]['increment variable name']).toBeDefined();

        result = f({name: 'Jeff', age: 19}, 
    ...
    describe('increment_step', () => {
      ...
      it('Should (de/in)crease the count any number', () => {
        let result = f([{name: 'Jeff', age: 19}, {name: 'Maria', age: 20}], 
          {headers: 'person', start: 20, increment_step: 20});
        expect(result[0].id).toBe(20);
        expect(result[1].id).toBe(40);

        result = result = f([{name: 'Jeff', age: 19}, {name: 'Maria', age: 20}],
          {headers: 'person', start: 189.75, increment_step: 1.25});
        expect(result[0].id).toBe(189.75);
        expect(result[1].id).toBe(191);

        result = result = f([{name: 'Jeff', age: 19}, {name: 'Maria', age: 20}, {}, {}],
          {headers: 'person', start: 10, increment_step: -5});
        ...
        result = f([{name: 'Jeff', age: 19}, {name: 'Maria', age: 20}],
          {headers: 'person', start: 20, increment_step: "20"});
        expect(result[0].id).toBe(20);
    ...
    it('Should use given function', () => {
        ...
        let result = f(demoData, {custom_id_function: customFunction});
        expect(result[0].id).toBeDefined();
        expect(result[1].id).toBeDefined();
        ...
        result = f(demoData, {custom_id_function: customFunction});
        expect(result[0].id).toBeDefined();
        expect(result[1].id).toBeDefined();
    ...
    it('Should use default function if the passed is incorrect', () => {
        ...
        const result = f(demoData, {custom_id_function: customFunction});
        expect(result[0].id).toBeDefined();
        expect(result[1].id).toBeDefined();
